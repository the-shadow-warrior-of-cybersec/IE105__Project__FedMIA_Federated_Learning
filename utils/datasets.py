...
def get_data(dataset, data_root, iid, num_users,data_aug, noniid_beta):
    ...
    if ds == 'dermnet': # Not available now
        print("Dermnet dataset is not available now")
        exit(1)
    ...
    if ds == 'oct': # Not available now
        print("OCT dataset is not available now")
        exit(1)
...
class DatasetSplit(Dataset):
    ...
    def __init__(self, dataset, idxs):
        if isinstance(dataset, DatasetSplit):
            ...
            # Nếu dataset không phải là DatasetSplit, tạo một bản sao mới
